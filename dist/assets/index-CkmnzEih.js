(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=[{id:`dice`,name:`Dice Roller`,icon:`🎲`},{id:`coin`,name:`Coin Flip`,icon:`🪙`},{id:`password`,name:`Password Gen`,icon:`🔑`},{id:`color`,name:`Color Picker`,icon:`🎨`},{id:`timer`,name:`Timer`,icon:`⏱️`},{id:`notes`,name:`Notepad`,icon:`📝`},{id:`calculator`,name:`Calculator`,icon:`🧮`},{id:`quotes`,name:`Random Quotes`,icon:`💬`},{id:`trivia`,name:`Trivia`,icon:`❓`},{id:`weather`,name:`Weather`,icon:`🌤️`},{id:`tracker`,name:`Daily Tracker`,icon:`📅`},{id:`browser`,name:`Web Browser`,icon:`🌐`},{id:`imagegen`,name:`Image Generator`,icon:`🖼️`}],t=0,n=10,r=new Map;function i(){return++t}function a(){return++n}function o(e){e.innerHTML=`
  ...
  `,t.style.left=e.clientX+`px`,t.style.top=e.clientY+`px`,t.querySelectorAll(`.context-menu-item`).forEach(e=>{e.addEventListener(`click`,()=>{W()})}),document.body.appendChild(t)}function W(){document.querySelectorAll(`.context-menu`).forEach(e=>e.remove())}function G(){return document.getElementById(`taskbar-items`)}function K({id:e,app:t}){let n=document.createElement(`div`);n.className=`taskbar-item active`,n.dataset.winId=e,n.innerHTML=`<div class="tb-icon" style="background:none;font-size:14px;display:flex;align-items:center;justify-content:center;">${t.icon}</div><span>${t.name}</span>`,n.addEventListener(`click`,()=>{let t=r.get(e);t&&(t.minimized?(L(e),F(e)):t.el.classList.contains(`active`)?I(e):F(e))}),G().appendChild(n)}function q({id:e}){let t=G().querySelector(`[data-win-id="${e}"]`);t&&t.remove()}function J({id:e}){document.querySelectorAll(`.taskbar-item`).forEach(t=>{t.classList.toggle(`active`,+t.dataset.winId===e)})}function Y(){j.on(`window:opened`,K),j.on(`window:closed`,q),j.on(`window:focused`,J)}function X(){let e=document.getElementById(`clock`);e.textContent=new Date().toLocaleTimeString([],{hour:`2-digit`,minute:`2-digit`})}function Z(){Y(),B(),V(),Q(),X(),setInterval(X,1e3)}function Q(){let e=document.getElementById(`start-button`),t=document.getElementById(`start-menu`);e.addEventListener(`click`,e=>{e.stopPropagation(),H()}),document.addEventListener(`click`,n=>{!t.classList.contains(`hidden`)&&!t.contains(n.target)&&!e.contains(n.target)&&H(!1),W()}),document.getElementById(`desktop`).addEventListener(`contextmenu`,U),document.addEventListener(`keydown`,e=>{e.key===`Escape`&&(H(!1),W())})}document.addEventListener(`DOMContentLoaded`,Z);